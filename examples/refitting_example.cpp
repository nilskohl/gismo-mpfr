...
int main(int argc, char *argv[])
{
    ...
    for (size_t p=0; p!=mp0->nPatches(); p++)
    {
        ...
        a = ab.col(0);
        b = ab.col(1);
        np.setConstant((ceil(pow(npts,1./mp.parDim()))));
    ...
    if (Hausdorff)
    {
        ...
        for (size_t p = 0; p!=hausdorffs.size(); p++)
        {
            ...
            funcs.at(p) = gsConstantFunction<>(hausdorffs.at(p) / sqrt(area),2);
        ...
        hausdorff(0,0) = std::accumulate(hausdorffs.begin(),hausdorffs.end(),static_cast<real_t>(0)) / hausdorffs.size();
